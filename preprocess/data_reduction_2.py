...
## Data frame of case info
    ...
    for idx in range(0, len(cases)-1):
        ...
        caseID = re.search(r'^VCU_Lung_\d+_\d', case).string
        ...
        try:
            ...
            device = torch.device('cpu')
            #device = torch.device('cuda:0')
            w17 = get_torch_tensor(w11, device).long()
            w18 = torch.nn.functional.one_hot(w17, 8)
            ...
            vols_red = dict.fromkeys(oars[:-1],0)
            for i6, oar_name in enumerate(oars):
                vols_red[oar_name] = w19[labels2[oar_name]-1].item()
                
            row_data = {**row_data1, **vols_red}
            df_cases.loc[len(df_cases)] = row_data   
            
           # filename_npz = os.path.join(out_dir, caseID+'_final.npz')
           # np.savez(filename_npz, CT=dict_np['CT'], DOSE=dict_np['DOSE'], OAR=oar_new, PTV=dict_np['PTV'], HIST_SIG=w14, HIST_CLINICAL = w16, BINS=dict_np['BINS'])
    ...
    #fh.close()    
    # debug
    return df_cases, vols_red

def batch_process_cases_1(in_dir, out_dir):
    ## generate predicted dose and GS dose and write them to Matlab M files
    # Initialize parser
# =============================================================================
#     parser = argparse.ArgumentParser()
#     
...
## Data frame of case info
    column_names = ['CaseID', 'PlanInd', 'PlanID', 'Dx','AllDataSet?','AllContours?','ExtractedOK?'] + oars
    df_cases = pd.DataFrame(columns=column_names)
    filename_caseinfo = os.path.join(out_dir, 'Cases_processing_to_mat.csv')
    #fh = open(filename,"w",newline="")
    in_dir_npz = "Z:/LulinY/Lung-dosimetrics/2024/python_code/AI_RTP/VCU_Lung_2024_processed_data"
    in_dir_npz = 'C:\Lulin-home\KBP-lung\CE project\AI_RTP\VCU_Lung_2024_processed_data'
    #for idx, case in enumerate(cases):
    for idx in range(0, 1): #len(cases)-1):
        case = cases[idx]
        if not case.endswith('.nrrd'):
            continue
        print('Processing case {}: {} of {} ...'.format(case, idx+1, len(cases)))
        case_path = os.path.join(in_dir, case)
        caseID = re.search(r'^VCU_Lung_\d+_\d', case).group()
        file_npz = os.path.join(in_dir_npz, caseID+'.npz')        
        row_data1 = {'CaseID':caseID}
        try:
            img = sitk.ReadImage(case_path)
            dose_fake = sitk.GetArrayFromImage(img)            
            
            dict_np = np.load(file_npz)        
            oar = dict_np['OAR']
            ptv = dict_np['PTV']
            ct = dict_np['CT']
            dose_real = dict_np['DOSE']
            
            hist_clinical = dict_np['HIST_CLINICAL']
            
            hist_fake, bin_fake = get_dvh_clinical(dose_fake, oar, ptv)
            
            filename_mat = os.path.join(out_dir, caseID+'_pred.mat')
            sio.savemat(filename_mat, {'DOSE_REAL': dose_real, 'DOSE_PRED': dose_fake, 'CT': ct, 'HIST_REAL': hist_clinical, 'HIST_PRED': hist_fake, 'BINS': bin_fake})
    ...
    #fh.close()    
    # debug
    return df_cases, hist_fake
## DEbug
out_dir = "Z:/LulinY/Lung-dosimetrics/2024/python_code/AI_RTP/VCU_Lung_2024_processed_data"
in_dir = "Z:/LulinY/Lung-dosimetrics/2024/python_code/AI_RTP/VCU_Lung_2024_processed_data"
in_dir = "G:/My Drive/AI_RTP/results/MAE_Moment_loss/test_200/npz_images"
## preditected Dose matrix by model
in_dir = 'G:/My Drive/AI_RTP/results/MAE_Moment_loss/test_latest/npz_images'
out_dir = 'C:/Lulin-home/KBP-lung/CE project/AI_RTP/matlab/matlab_6'
#in_dir = "C:\Lulin-home\KBP-lung\CE project\AI_RTP\VCU_Lung_2024_processed_data"
#in_dir = "Z:\\LulinY\\Lung-dosimetrics\\2024\\python_code\\AI_RTP\\results\\Test_200\\npz_images"
#out_dir = "Z:\\LulinY\\Lung-dosimetrics\\2024\\python_code\\AI_RTP\\results\\Test_200\\matlab"
w2, hist = batch_process_cases_1(in_dir,out_dir)
#fh.close()