...
class DosePrediction3DModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        # print('dropout value: ', not opt.no_dropout)
        # Turning off dropout
        self.netG = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.norm,
                                      not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
        ...
        self.criterionMAE = torch.nn.L1Loss().to(self.device)
    ...
    # def save_weights(self, opt):
    #     weight_dir = os.path.join(opt.checkpoints_dir, opt.name)
    #     weight_filename = os.path.join(weight_dir, 'weights_path_name.pth')
    #     torch.save(self.netG.state_dict(), weight_filename)
    ...
    def backward_G(self):
        ...
        self.loss_G_MOMENT = 0.05*self.criterionMoment(self.fake_Dose, self.real_CT[:, 1:, ...], self.real_Dose)  # Changed for  beam(Gourav)
        ...
        self.loss_G = self.loss_G_MAE +self.loss_G_MOMENT  # self.loss_G_DVH  ##+  # Could add other losses here like DVH,
        # RMSE or SSIM
    ...
    def calculate_validation_loss(self):
        ...
        # self.loss_G_MSE = self.criterionMSE(self.fake_Dose, self.real_Dose) * self.opt.lambda_L1
        # self.loss_G_DVH = 10*self.criterionDVH(self.fake_Dose, self.target_hist, self.target_bins, self.real_CT[:, 1:,...])#Changed(Gourav)
        # self.loss_G_MOMENT = 0.05 * self.criterionMoment(self.fake_Dose, self.real_CT[:, 1:, ...], self.real_Dose)  # Change
        self.loss_G = self.loss_G_MAE #+ self.loss_G_DVH#+self.loss_G_MOMENT
        print(self.loss_G)
